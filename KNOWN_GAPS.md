...
1. **Fixed enum, not admin-configurable.** `phase` is a `CHECK` constraint on 5 values (`foundation`, `structure`, `mep`, `finishing`, `handover`), not a lookup table. Note: at the time this was written, the "admin-configurable like `work_categories`" comparison in the original ask didn't hold, since `work_categories` wasn't a real table either — see the entry below, since `work_categories` has since been built (2026-07-24) as an actual admin-configurable lookup table, unlike `site_phases`.
...
## BUILT — Admin-configurable work categories (work_categories)

**Where:** `supabase/migrations/20260724080001_work_categories.sql`, `src/hooks/useWorkCategories.ts`, `src/hooks/useCreateWorkCategory.ts`, `src/hooks/useUpdateWorkCategory.ts`, `src/routes/WorkCategories.tsx` (new `/work-categories` route), nav entries in `src/routes/ProtectedLayout.tsx` and `src/components/MobileBottomNav.tsx`. Consumers updated: `src/routes/Labour.tsx`, `src/routes/Attendance.tsx` (both previously had a duplicated hardcoded `WORK_CATEGORIES` array, now removed).

**Design decisions (proposed and confirmed before building, per the working agreement):**
1. **`default_work_category` (labour) / `work_category` (labour_attendance) stay plain TEXT, not FK.** Both were already unconstrained `TEXT` with no CHECK-enum. An FK (especially with `ON UPDATE CASCADE` to support renames) would let a category rename silently rewrite already-settled, already-paid historical attendance/payroll text — exactly the kind of retroactive mutation of financial history CLAUDE.md warns against. `work_categories` is purely the source of *options offered in the UI*, not a referential-integrity constraint on historical rows.
2. **`name` is immutable after creation.** No UPDATE path in the app (hook, form, or RLS) changes `name` — only `is_active` toggles. "Renaming" means retiring the old row and creating a new one, so historical records keep their original recorded text forever, unambiguously.
3. **`created_by`/`last_edited_by` are nullable**, unlike most audit columns in this codebase (which are `NOT NULL`). The 6 seeded rows (`mason`, `helper`, `electrician`, `painter`, `carpenter`, `plumber` — the exact values from the old hardcoded array) were not created by any human actor; inventing an admin UUID for them would misrepresent the audit trail. NULL honestly means "system-seeded." Every row created through the app's create-hook sets both fields to the real user.
4. **Permissions** (per CLAUDE.md Section 5's pre-existing but previously unimplemented "Manage work categories ✓ ✓ ✗" row): SELECT all three roles (supervisors need this to populate the attendance dropdown), INSERT/UPDATE Admin + Office Manager only, no DELETE policy for anyone (retirement is `is_active = false`).
5. **New dedicated `/work-categories` route**, not folded into the existing `/users` page — `Users.tsx` is narrowly scoped to supervisor/site-assignment/wage-visibility concerns (documented as a 3-phase build in its own header comment) and bolting an unrelated lookup-table CRUD screen onto it would cramp a page already juggling two concerns. Mirrors how `/users` already gets its own top-level nav slot rather than living under a settings umbrella that doesn't exist elsewhere in this app.
6. **Retired-category display, not just filtering.** The attendance grid and the labour edit form must keep showing a worker's/record's already-recorded category even after it's retired (e.g. an old attendance row that says "mason" must still show "mason", not blank out, if "mason" is later retired) — while the dropdown should not *offer* retired categories for new selections. Implemented via a shared `getCategoryOptions(categories, currentValue)` helper in `useWorkCategories.ts`: returns all active categories, plus the current value appended (labelled "(retired)") only if it isn't already active. Applied at the 3 call sites that display an existing stored value (`Labour.tsx`'s `default_work_category` edit select, `Attendance.tsx`'s mobile and desktop `work_category` selects); the 2 call sites that only ever pick a category for a brand-new assignment (`Labour.tsx`'s `task_category` fields) just use the active list directly, since there's no historical value to preserve there.

**Verification:** `npx supabase migration up` applied cleanly against the existing local DB (no reset, preserving prior test data); confirmed all 6 seed rows present with `is_active = true` and `created_by`/`last_edited_by` both `NULL` via direct `psql`. `npx supabase gen types typescript --local` regenerated `src/types/database.ts` with only additive changes (47 insertions, 0 deletions/modifications elsewhere). `tsc -b`/`tsc --noEmit` clean after all frontend changes. Confirmed no remaining `WORK_CATEGORIES` references in any file that's part of the build (`src/routes/Labour.tsx.backup` still has the old array, but that file is an unregistered stray backup, not part of routing — left alone, flagged separately).

**Status:** BUILT on 2026-07-24.
