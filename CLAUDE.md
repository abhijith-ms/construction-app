...
- `work_categories` — admin-configurable list of job types (mason, helper, ...). `name` immutable after creation; retirement via `is_active = false`. `default_work_category`/`work_category` below stay plain TEXT (not FK) so historical attendance/payroll text never changes if a category is retired.
...
- BUILT 2026-07-24 — Admin-configurable work categories. New `work_categories` table (migration `20260724080001_work_categories.sql`) replaces the hardcoded `WORK_CATEGORIES` array duplicated in `src/routes/Labour.tsx` and `src/routes/Attendance.tsx`. `name` is immutable after creation (retire via `is_active = false` + create new instead of renaming); `default_work_category`/`work_category` deliberately stay plain TEXT, not FK, so historical attendance/payroll records never silently change if a category is retired. New `/work-categories` admin page (Admin + Office Manager only, per Section 5). See KNOWN_GAPS.md for full design rationale and verification detail.
